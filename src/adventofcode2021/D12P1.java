...
public class D12P1 implements PuzzleBasics {
    ...
    private int backtracking(List<Paths> paths, List<Cave> caves, String caveName) {
        ...
        for (Paths path : paths) {
            if (path.hasCave(caveName)) {
                ...
                if (!cave.isOneVisit() || (cave.isOneVisit() && !cave.isVisited())) {
                    if (cave.isOneVisit()) {
                        cave.setVisited(true);
                        caves.set(caveIndex, cave);
                        nrOfPaths = nrOfPaths + backtracking(paths, caves, otherCaveName);
    ...
    public void run() throws IOException {
        ...
        System.out.println(backtracking(paths, caves, "start"));
    }
}
